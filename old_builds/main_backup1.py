...
tile_size = 50
tile_scale = 1
screen_size = (int(window_size[0]*tile_size*tile_scale), int(window_size[1]*tile_size*tile_scale))
...
class Tile:
    def __init__(self, image, edges, size=50, scale=1):
        ...
        self.image = pg.transform.scale(image, (self.size_constant, self.size_constant))
        self.edges = edges
    ...
    def rotate(self, num):
        ...
        for i in range(len(self.edges)):
            new_edges[i] = self.edges[(i - num + len(self.edges)) % len(self.edges)]
        return Tile(new_image, new_edges, self.size, self.scale)

def load_demo_set_full():
    global tiles
    tiles.append(Tile(pg.image.load('images/demo_tiles/blank.png'), [0, 0, 0, 0], tile_size, tile_scale))
    tiles.append(Tile(pg.image.load('images/demo_tiles/up.png'), [1, 1, 0, 1], tile_size, tile_scale))
    tiles.append(tiles[1].rotate(1))
    tiles.append(tiles[1].rotate(2))
    tiles.append(tiles[1].rotate(3))
    tiles.append(Tile(pg.image.load('images/demo_tiles/corner.png'), [1, 1, 0, 0], tile_size, tile_scale))
    tiles.append(tiles[5].rotate(1))
    tiles.append(tiles[5].rotate(2))
    tiles.append(tiles[5].rotate(3))
    tiles.append(Tile(pg.image.load('images/demo_tiles/cross.png'), [1, 1, 1, 1], tile_size, tile_scale))
...
def blit(tile):
    ...
    window.blit(tiles[tile['index']].image, (image_size*tile['grid_pos'][0], image_size*tile['grid_pos'][1]))
